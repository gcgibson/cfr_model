...
timezero_day <-  "2020-12-04"
forecast_day <-  as.Date(timezero_day) + 27
region <- "ma"
...
X_new <- seq(length(X)+1,length(X)+27)
...
B_predict <- B_total[1:num_basis,seq(length(X)+1,length(X)+27)]
num_data <- length(X);
...
fit<-sampling(sm,iter=100,chains=1,data=list(y_deaths=log(pmax(0,ca_deaths$value)+1),
