data { 
  ...
  matrix[num_data+27,num_data+27] mask_pred;
...
transformed parameters { 
  ...
  for (i in 2:num_basis){
    tmp_var = if_else(i < num_basis-5,tau,tau/100);
  ...
  pred_death = (mask*Y_hat);
} 
 
model { 
  
  cfr_transform[1] ~ normal(1,.1);
  for (i in 2:num_data){
  ...
  tau ~ normal(0, 10); 
  sigma ~ cauchy(0, 1); 
  y_cases ~ normal(Y_hat, 1000);
  y_deaths ~ normal(pred_death,1);
} 

generated quantities {
  vector[27] predicted_y_hat;
  row_vector[num_basis] transformed_a;
  vector[num_data+27] cfr_transform_new;

  vector[num_data +27] predicted_death;
  
#  transformed_a = to_row_vector(rep_vector(mean(a[(num_basis-20):num_basis]),num_basis));
  cfr_transform_new = to_vector(append_array(cfr_transform,rep_array(cfr_transform[num_data],27)));
  predicted_y_hat = a0*X_new + to_vector(a*B_predict);
  predicted_death =   (mask_pred*to_vector(append_array(to_array_1d(Y_hat),to_array_1d(predicted_y_hat))));
